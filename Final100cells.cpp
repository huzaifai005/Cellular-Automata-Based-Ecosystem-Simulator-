//This is the final 100 cells base model
...
public:
    ...
    Animal(int r_coord, int c_coord, EntityType type, char maleSymbol, char femaleSymbol, Gender gender_val, // Renamed parameter
        ...
        : Entity(r_coord, c_coord, type, (gender_val == Gender::MALE ? maleSymbol : femaleSymbol)), // Used renamed parameter
          ...
          visionRange(visionRange_val), moveCost(moveCost_val), reproductionCooldown(4), currentReproductionCooldown(0), // Reduced cooldown
    ...
    bool canReproduce() const {
        return gender == Gender::FEMALE && !isPregnant && age >= minBreedingAge &&
               energy >= energyToReproduce && currentReproductionCooldown == 0;
    }
};
    ...
    void move(Grid& grid, MonthlyStats& stats) override {
        ...
        if (energy < maxEnergy * 0.9) {
            ...
            if (!plantsNearby.empty()) {
                ...
                if (closestPlant) {
                    int dr_move = (closestPlant->r > r) ? (closestPlant->r - r) : ((closestPlant->r < r) ? (closestPlant->r - r) : 0);
                    int dc_move = (closestPlant->c > c) ? (closestPlant->c - c) : ((closestPlant->c < c) ? (closestPlant->c - c) : 0);
                    int nr = r + dr_move;
                    int nc = c + dc_move;
   ...
   void move(Grid& grid, MonthlyStats& stats) override {
    ...
    if (energy < maxEnergy * 0.85) {
        ...
        if (!herbivoresNearby.empty()) {
            ...
            if (closestHerbivore) {
                int dr_move = (closestHerbivore->r > r) ? (closestHerbivore->r - r) : ((closestHerbivore->r < r) ? (closestHerbivore->r - r) : 0);
                int dc_move = (closestHerbivore->c > c) ? (closestHerbivore->r - r) : ((closestHerbivore->c < c) ? (closestHerbivore->r - r) : 0);
                int nr = r + dr_move;
                int nc = c + dc_move;
